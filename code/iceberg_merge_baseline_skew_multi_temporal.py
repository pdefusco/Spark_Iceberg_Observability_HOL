...
spark = SparkSession.builder \
    .appName("MergeIntoSparkApp1B_DH_multi_temporal_skew") \
    .getOrCreate()
...
# Add temporal skew across multiple dates
df1 = df.withColumn(
    "event_ts",
    when(rand() < 0.4, to_date(lit("2020-01-01")))  # 40% of rows
    .when(rand() < 0.7, to_date(lit("2020-01-02")))  # +30% (70% total so far)
    .when(rand() < 0.85, to_date(lit("2020-01-03")))  # +15% (85%)
    .otherwise(expr(f"date_add(to_date('{base_ts}'), id % 30)"))  # remaining 15% spread across 30 days
)

# Add other columns (optional)
df1 = df1.withColumn("category", expr("CASE id % 5 WHEN 0 THEN 'A' WHEN 1 THEN 'B' WHEN 2 THEN 'C' WHEN 3 THEN 'D' ELSE 'E' END")) \
